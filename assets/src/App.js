...
function App() {
    ...
    const [loadUsersButtonDisplay, setLoadUsersButtonDisplay] = useState(false);
    const [loadDetailsButtonDisplay, setLoadDetailsButtonDisplay] = useState(false);
    const [loadingDisplay, setLoadingDisplay] = useState(true);
    const fetchData = async (url, isDetails = false) => {
                ...
                <div className="row">
                    <div className="col">
                        ...
                        {users.length > 0 && usersDisplay && (
                            <UsersTable users={users} openDetailsClick={handleOpenDetailsClick}/>
                        )}
                        {!users.length && usersDisplay && !detailsDisplay && !error && !loadingDisplay && (
                            <div>
                                <p>No users found</p>
                                {!loadUsersButtonDisplay && (
                                    <Button title="Reload Users Table" onClick={handleUsersLoadButton} />
                                )}
                            </div>
                        )}
                        ...
                        {loadUsersButtonDisplay && (
                            <Button title="Load Users Table" onClick={handleUsersLoadButton} />
                        )}
                        {loadDetailsButtonDisplay && (
                            <Button title="Reload User Details" onClick={handleDetailsLoadButton} />
                        )}
                        {loadingDisplay && (
                            <Loader />
                        )}
