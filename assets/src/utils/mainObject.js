export default class MainObject {
	constructor() {
		...
		this.ajaxUrl = this.isReactDev
			? 'https://jsonplaceholder.typicode.com/'
			: this.getGlobalProperty( 'ajaxUrl', '' );
		this.nonce = this.isReactDev
			? ''
			: this.getGlobalProperty( 'nonce', '' );
	...
	getGlobalProperty( property, defaultValue = '' ) {
		return this.isReactDev
			? defaultValue
			: window?.global_user_atlas?.[ property ] || defaultValue;
	}
